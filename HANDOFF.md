...
- **Section reachability:** a section shows when its `types` match (AND its `showIf`); `alsoShowIf: { key, equals }` is an OR escape hatch so a **gate** can surface a section regardless of which type was checked — `injury` on `anyoneInjured`=Yes, `tow` on `towRequired`=Yes, the accident/other-parties section on `otherPartyInvolved`=Yes. This fixes fields that were unreachable when a gate answered the underlying question but the matching type box wasn't checked (`sectionVisible`).
