...
- **Pending: FIX 1b** (case number stored as text) is **not yet in `incident-api-v2.gs`** — after `appendRow`, `setNumberFormat('@').setValue(caseNumber)` on column A so leading zeros survive (`08042601`, not `8042601`). Needs a backend redeploy.
