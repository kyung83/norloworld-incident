...
- **Screen order:** `anyoneInjured` → `otherPartiesInjured` (only if `anyoneInjured` = Yes) → `pedestrianInvolved` → identity → remaining gates → incident types → visible branch sections + the checklist → review. The gates before identity are `LEADING_GATES` (in order); identity comes early so an abandoned form still identifies the driver. Gate-level `showIf` is honored (`gateVisible`).
- **Resume, not auto-restore:** the form always opens at step one; a saved draft is **offered** via a banner (driver name + truck + "saved X ago", "Pick up where I left off" / two-tap "Start over"). Drafts expire after **6h** (`DRAFT_MAX_AGE_MS`) and are discarded on load; resuming restores the `sessionId` so photos stay in one Drive folder. Submitting clears the draft.
- **Gates** = severity questions that drive tiering. Most are always shown; a couple are conditional: `otherPartiesInjured` only when `anyoneInjured` = Yes, and `otherPartyInvolved` is **skipped when `otherVehicleInvolved` = Yes** (auto-set to "Yes" so the backend's fail-upward check still reads it in the payload) and otherwise **asked reworded** ("Did you hit anything that isn't ours?"). Gate `showIf` supports `equals` and `notEquals` (`gateVisible`). The first gate shows a banner with **Driver Line: (989) 802-7135** (`tel:+19898027135`) — this dials what it displays; leave it.
- **Incident types:** `accident`, `animalStrike`, `injury`, `damageOurs`, `damageTheirs`, `tow`, `other`. `animalStrike` (deer, etc.) replaces the old `deerAnimal` field in the accident section; it shows the `damageOurs` detail + the checklist (always-required scene/our-equipment photos). (Historical: old keys were `vehicleDamage`→`damageOurs`, `propertyDamage`→`damageTheirs`; old sheet rows use old labels.)
- **Section reachability:** a section shows when its `types` match (AND its `showIf`); `alsoShowIf` is an OR escape hatch so a **gate** can surface a section regardless of which type was checked. It's one `{ key, equals }` or an **array** of them (any match). Uses: `injury` on `anyoneInjured`=Yes; `tow` on `towRequired`=Yes; `otherParties` on `otherPartyInvolved`=Yes; `witnesses` on `otherPartyInvolved`=Yes **or** `policeOnScene`=Yes. Fixes fields that were unreachable when a gate answered the underlying question but the matching type box wasn't checked (`sectionVisible`).
...
- **Photo upload** (per-photo model): state is `uploads: key -> [{status,url,dataUrl}]`. Fields with `multiple: true` in the schema (scene, our-equipment, their-property, their-vehicle, freight-damage) take several photos with thumbnails / add-another / per-photo retry / remove; document fields stay single. capture → downscale (1600px, JPEG 0.6) → concurrency limiter (`UPLOAD_CONCURRENCY = 3`) → `savePhoto` (indexed slot `key-2` for the 2nd+) → 60s timeout + 2 backed-off retries; **Retry** reuses the in-memory image. A sync effect mirrors done URLs into `values[key]` as an **array** (backend `photoCell_` joins newline-separated; `allPhotoUrls_` flattens for folder/lane checks). Submit disabled while any upload is in flight. Image data isn't persisted to the draft; on **resume**, `uploads` is rebuilt from the saved URLs (thumbnail shows "Uploaded", no image).
