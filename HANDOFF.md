...
- **Screen order:** first gate (`anyoneInjured`) → `otherPartiesInjured` (only if `anyoneInjured` = Yes) → identity → remaining gates → incident types → visible branch sections + the checklist → review. Identity comes early so an abandoned form still identifies the driver. Gate-level `showIf` is honored by the step builder (`gateVisible`).
